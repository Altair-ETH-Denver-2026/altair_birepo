          ...
          return {
            ...
            swap: swap
              ? {
                  ...
                  chain: typeof swap.chain === 'string' ? swap.chain : null,
                  sellToken: typeof swap.sellToken === 'string' ? swap.sellToken : null,
                  buyToken: typeof swap.buyToken === 'string' ? swap.buyToken : null,
                  sellAmount: typeof swap.sellAmount === 'string' ? swap.sellAmount : null,
                  buyAmount: typeof swap.buyAmount === 'string' ? swap.buyAmount : null,
...
export async function appendSwapToHistory(params: AppendSwapParams): Promise<ArchiveResult> {
  const { CID, chain, sellToken, buyToken, sellAmount, buyAmount, txHash, accessToken, timestamp } = params;
  if (ZG_VERBOSE) {
