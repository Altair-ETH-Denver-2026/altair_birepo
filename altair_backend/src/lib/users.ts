...
export async function syncUserFromAccessToken(accessToken: string) {
  ...
  const evmAddress = extractEvmAddress(user);
  const solAddress = extractSolAddress(user);
  const linkedAccounts = extractLinkedAccounts(user);
  const email = extractEmail(user);
  const phone = extractPhone(user);
  const profileImageUrl = typeof user?.profile?.imageUrl === 'string' ? user.profile.imageUrl : null;
  const embeddedWalletId = typeof user?.wallet?.id === 'string' ? user.wallet.id : null;
