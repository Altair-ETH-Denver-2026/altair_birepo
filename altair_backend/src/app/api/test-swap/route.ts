...
const formatAmountFromRaw = (raw: string, decimals: number): string => {
...
const resolveBuyAmount = async (params: {
  ...
  if (buyToken === 'ETH') {
    const startBalance = await withWaitLogger(
      {
        file: 'altair_backend1/src/app/api/test-swap/route.ts',
    ...
    const endBalance = await withWaitLogger(
      {
        file: 'altair_backend1/src/app/api/test-swap/route.ts',
        target: 'EVM getBalance',
    ...
    const effectiveGasPrice = 'effectiveGasPrice' in receipt && receipt.effectiveGasPrice
    ...
    if (delta <= 0n) {
      throw new Error('Unable to resolve ETH buy amount from balance delta.');
    }
    return delta.toString();
  }
...
export async function POST(req: Request) {
  try {
    ...
    if (txHash && normalizedBuyToken && normalizedSellToken && amount) {
      ...
      const buyAmountRaw = await withWaitLogger(
        {
          file: 'altair_backend1/src/app/api/test-swap/route.ts',
