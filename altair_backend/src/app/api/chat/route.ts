    ...
    ? params.recentSwaps
        ...
        .map((entry) => {
          ...
          return {
            ...
            chain: typeof item.chain === 'string' ? item.chain : null,
            sellToken: typeof item.sellToken === 'string' ? item.sellToken : null,
            buyToken: typeof item.buyToken === 'string' ? item.buyToken : null,
            sellAmount: typeof item.sellAmount === 'string' ? item.sellAmount : null,
            buyAmount: typeof item.buyAmount === 'string' ? item.buyAmount : null,
...
export async function POST(req: Request) {
  try {
    ...
    if (typeof resolvedAccessToken === 'string' && resolvedAccessToken.length > 0) {
      ...
      try {
        ...
        const [userDoc, swapDocs, chatDocs] = await withWaitLogger(
          ...
          () =>
            Promise.all([
              User.findOne({ UID: user.UID }, { balances: 1 }).lean(),
              Swap.find({ UID: user.UID })
                .select({
                  SID: 1,
                  CID: 1,
                  chain: 1,
    ...
    console.log('runningSummary: aiResponse', aiResponse);
    hadSwapExecution = aiResponse.includes('"type"') && aiResponse.includes('SWAP_INTENT');
